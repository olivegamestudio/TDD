...
**A sprite keeps its texture's shape, and `Stretch` is the one exception.** `Sprite.Scale` is a
single factor because almost everything drawn is a thing in the world, and a thing in the world has
a shape. What needs the exception is anything sized to the *window*: a window's aspect is whatever
the player dragged it to, so an overlay covering one cannot be square, and a uniform scale can only
leave two edges bare or push the other two off the screen. `Stretch` defaults to one on both axes
and is multiplied into the scale by `MonoGameRenderer`, so `Scale` still means what it says
everywhere it is the only thing set. It sits outside the positional parameters for the reason
`Colour` does, above.

**The sky is tinted; what is flown through is not.** `RegionView.BackdropTint` reaches the bodies
painted on the sky — those authored with no parallax — and nothing else, and `GameScreen` sets it
from `GameScreen.BackdropBrightness` on entering. The split is a gameplay one rather than a
stylistic one: the backdrop is scenery the player looks past, while the debris is scenery the
player flies into, and dimming the second costs them the warning they get of what they are about to
hit. The tint lives on the view rather than in the region file because how dark the sky is is a
...
`IRenderable.Render`, so there is no seam through which it could take a press from what is under
it. The shape of the ramp is in the asset, generated by `scripts/make-vignette.py` so the radii are
reviewable rather than buried in a PNG; how heavily it is laid on is `Vignette.Strength`, in code,
because that is the half worth tuning without redrawing anything.
