...
| Pillar | Standing |
| ------ | -------- |
| 1. Flying feels good | Being exercised. The ship has thrust, a helm, momentum through a turn and a frame-rate-independent reach. A keyboard and a gamepad are bound, so a person can fly it. One thing still short of the pillar: triggers are sampled once a frame rather than swept, so a long enough frame can still step a fast ship over a marker. Quest 1's markers are sized clear of that at any playable frame rate. Whether flying it actually *feels* good is now a question someone can answer by playing it — the handling has never been felt by a person, only checked against Quest 1's distances. |
