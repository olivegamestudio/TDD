...
| Pillar | Standing |
| ------ | -------- |
| 1. Flying feels good | Being exercised. The ship has thrust, a helm, momentum through a turn and a frame-rate-independent reach, and triggers are swept across the ground a frame covered rather than sampled at the point it ended, so no frame length steps a fast ship over a marker. One thing still short of the pillar: no real input device is bound, so nobody can fly it outside a test. |
