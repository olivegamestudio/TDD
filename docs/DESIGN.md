...
| 4. The world was here first | Partly expressed, and the record has begun to separate from perishable position: the save now carries the ship the player owns (`SaveGame.ShipId`) alongside position and quest state. The ship is recorded rather than re-derived precisely because re-deriving would quietly take back a hull the player had earned — the failure this pillar exists to prevent — though with one hull in the game the two readings are still indistinguishable in practice. Experience, credits and quest history are still absent, and world progression currently advances from the game screen. The rule against silently discarding the record is honoured early: a save that could only not be *read* is played over but never written to, so a file locked for a moment is not replaced by a new game. A save that is genuinely *damaged* is still discarded, which is right while a save holds nothing precious and becomes the wrong answer as more of the record lands. |
