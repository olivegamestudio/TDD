...
trigger a fast ship flies straight through is a bug against this pillar, not a tuning detail, and
is answered by measuring the ground a frame covered rather than by widening the marker.
...
| Pillar | Standing |
| ------ | -------- |
| 1. Flying feels good | Not yet exercised. Nothing moves the ship in the running game; movement and physics are issue #3. Triggers no longer depend on being sized for a ship at speed: they are measured against the ground covered in a frame rather than the point it ended at, so no frame is long enough to step over one. |
