...
| Pillar | Standing |
| ------ | -------- |
| 1. Flying feels good | Being exercised. The ship has thrust, a helm, momentum through a turn and a frame-rate-independent reach, and triggers are swept across the frame rather than sampled at the end of it, so a marker fires at any frame length rather than relying on the markers being tolerant enough. Thrust and drag are now Aether.Physics2D — a real rigid body under the ship rather than a formula standing in for one — with frame independence held by a fixed-step accumulator instead of the closed-form integration that used to guarantee it; the helm stays the hand-rolled rate it always was, since turning was never modelled with inertia. The ship now has a hull, too: it collides with the debris field's rocks rather than flying through them, which is what the pillar's own language about the debris asks for — scenery the player has to see in time to avoid — but was not true of it until now. That surfaced something the field's own density had been hiding: quest 1's opening stretch has no comfortable straight route through it for a ship this size, only a route with almost no room either side, which is a question for a human looking at the field rather than something this change should decide alone. The ship is now on screen too, and the camera turns with it: the ship's nose is pinned to the top of the window and the world rotates around it, so a turn reads as the world swinging past rather than as a sprite spinning in place. A keyboard and a gamepad are now bound, so a person can fly quest 1 rather than only a test — which makes "does flying actually feel good?" answerable for the first time, and it has not been answered: the handling in `BattleForceShip` has only ever been checked against quest 1's distances, never felt, and now neither has the field it flies through. One thing still short of the pillar: with the camera holding the ship in the middle of an empty background, full thrust looks the same as a standstill until there is something fixed to read motion against. |
