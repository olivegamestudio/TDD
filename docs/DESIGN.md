...
| Pillar | Standing |
| ------ | -------- |
| 1. Flying feels good | Being exercised, and now flyable. The ship has thrust, a helm, momentum through a turn and a frame-rate-independent reach, and a keyboard and gamepad are bound to it, so Quest 1 can be completed by a person rather than only by a test. Still short of the pillar: triggers are sampled once a frame rather than swept, so a long enough frame can step a fast ship over a marker — Quest 1's markers are sized clear of that at any playable frame rate. Whether it actually *feels* good is now a question someone can answer by playing it, which it was not before. |
