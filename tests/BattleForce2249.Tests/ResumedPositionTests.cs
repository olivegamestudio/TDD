...
public sealed class ResumedPositionTests
{
    ...
    /// <summary>How close quest 1's end marker has to be for it to complete, so the flying tests can
    /// assert arrival without restating the authored distance as a coordinate.</summary>
    ...
    /// <summary>
    /// A save at <see cref="WellPastTheStartMarker"/> holding the given quest entries verbatim, so
    /// the shapes under test are the JSON a hand-edited or older build's file actually holds.
    /// </summary>
